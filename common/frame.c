...
static x264_frame_t *x264_frame_new( x264_t *h, int b_fdec )
{
    ...
    for( int i = 0; i < 2; i++ )
        PREALLOC( frame->i_row_satds[i], i_lines/16 * sizeof(int) );
    ...
    if( b_fdec ) /* fdec frame */
    {
        ...
        PREALLOC( frame->mv, 2*16 * i_mb_count * sizeof(int16_t) );
        PREALLOC( frame->mv16x16, 2*(i_mb_count+1) * sizeof(int16_t) );
        PREALLOC( frame->ref, 4 * i_mb_count * sizeof(int8_t) );
...
static void  pixel_memset( pixel *dst, pixel *src, int len, int size )
{
...
void x264_frame_expand_border( x264_t *h, x264_frame_t *frame, int mb_y )
{
    ...
    int b_end   = mb_y == h->i_threadslice_end - (1);
    if( mb_y & 0 )
        return;
    for( int i = 0; i < frame->i_plane; i++ )
    {
        ...
        int height = (pad_bot ? 16*(h->mb.i_mb_height - mb_y) : 16) >> v_shift;
        int padh = PADH;
