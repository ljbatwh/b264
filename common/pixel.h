...
typedef struct
{
    x264_pixel_cmp_t  sad[8];//from PIXEL_16x16 to PIXEL_4x16
    x264_pixel_cmp_t  ssd[8];//from PIXEL_16x16 to PIXEL_4x16
    x264_pixel_cmp_t satd[8];//from PIXEL_16x16 to PIXEL_4x16
    x264_pixel_cmp_t ssim[7];//do not use?
    x264_pixel_cmp_t sa8d[4];//PIXEL_16x16 and PIXEL_8x8
    ...
    //Absolute Sum Difference 8 times
    int (*asd8)( pixel *pix1, intptr_t stride1, pixel *pix2, intptr_t stride2, int height );

    //return = sum(of all pixel) + ((uint64_t)sqr(sum of all pixel sqr) << 32)
    uint64_t (*var[4])( pixel *pix, intptr_t stride );//PIXEL_16x16 to PIXEL_8x8
    //return = ssd - ((uint64_t)sum(abs sum of diff) * sum >> 7);
    int (*var2[4])( pixel *pix1, intptr_t stride1,
                    pixel *pix2, intptr_t stride2, int *ssd );//PIXEL_16x16 to PIXEL_8x8
    //hadamard transform and just use ac                    
    ...
    x264_pixel_cmp_x3_t sad_x3[7];//fec sad pix0 pix1 pix2
    x264_pixel_cmp_x4_t sad_x4[7];//fec sad pix0 pix1 pix2 pix3
