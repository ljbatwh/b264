...
static  void zigzag_scan_2x2_dc( dctcoef level[4], dctcoef dct[4] )
{
    ZIG(0,0,0)
    ZIG(1,0,1)
    ZIG(2,1,0)
...
static  void idct_dequant_2x2_dc( dctcoef dct[4], dctcoef dct4x4[4][16], int dequant_mf[6][16], int i_qp )
{
...
static  void idct_dequant_2x2_dconly( dctcoef dct[4], int dequant_mf[6][16], int i_qp )
{
    IDCT_DEQUANT_2X2_START
    dct[0] = (d0 + d1) * dmf >> 5;
    dct[1] = (d0 - d1) * dmf >> 5;
...
static  void dct2x2dc( dctcoef d[4], dctcoef dct4x4[4][16] )
{
...
static  int array_non_zero( dctcoef *v, int i_count )
{
    if( WORD_SIZE == 8 )
    {
...
static  int x264_mb_optimize_chroma_dc( x264_t *h, dctcoef *dct_dc, int dequant_mf[6][16], int i_qp)
{
    int dmf = dequant_mf[i_qp%6][0] << i_qp/6;
...
static  void x264_mb_encode_chroma_internal( x264_t *h, int b_inter, int i_qp)
{
    int nz, nz_dc;
            ...
            for( int ch = 0; ch < 2; ch++ )
            {
                if( ssd[ch] > thresh )
                {
...

                    {
                        nz_dc = 0;
                        for( int i = 0; i <= 0; i++ )
                            nz_dc |= h->quantf.quant_2x2_dc( &dct_dc[4*i], h->quant4_mf[CQM_4IC+b_inter][i_qp+3*0][0] >> 1,
                                                             h->quant4_bias[CQM_4IC+b_inter][i_qp+3*0][0] << 1 );
                    }

                    if( nz_dc )
                    {
                        if( !x264_mb_optimize_chroma_dc( h, dct_dc, dequant_mf, i_qp+3*0) )
                            continue;
    ...
    for( int ch = 0; ch < 2; ch++ )
    {
        ...
        if( h->mb.b_lossless )
        {
            ...
            for( int i = 0; i < (4); i++ )
            {
                ...
                nz = h->zigzagf.sub_4x4ac( h->dct.luma4x4[16+i+(0?i&4:0)+ch*16], p_src+oe, p_dst+od,
                                           &h->dct.chroma_dc[ch][i] );
                h->mb.cache.non_zero_count[x264_scan8[16+i+(0)+ch*16]] = nz;
...
{
        x264_mb_encode_chroma_internal( h, b_inter, i_qp);
}
 ...
 *****************************************************************************/
static  void x264_macroblock_encode_internal( x264_t *h, int plane_count, int chroma )
{
    int i_qp = h->mb.i_qp;
    ...
    if( h->mb.i_type == P_SKIP )
    {
        ...
        if( !h->mb.b_skip_mc )
        {
            int mvx = x264_clip3( h->mb.cache.mv[x264_scan8[0]][0],
                                  h->mb.mv_min[0], h->mb.mv_max[0] );
            int mvy = x264_clip3( h->mb.cache.mv[x264_scan8[0]][1],
                                  h->mb.mv_min[1], h->mb.mv_max[1] );

            for( int p = 0; p < plane_count; p++ )
                h->mc.mc_luma( h->mb.pic.p_fdec[p], FDEC_STRIDE,
                               &h->mb.pic.p_fref[0][p*4], h->mb.pic.i_stride[p],
                               mvx, mvy, 16, 16 );
            ...
            if( chroma )
            {
                ...
                if( mvx | mvy )
                    h->mc.mc_chroma( h->mb.pic.p_fdec[1], h->mb.pic.p_fdec[2], FDEC_STRIDE,
                                     h->mb.pic.p_fref[0][4], h->mb.pic.i_stride[1],
                                     mvx, 2*mvy>>v_shift, 8, height );
                else
                    h->mc.load_deinterleave_chroma_fdec( h->mb.pic.p_fdec[1], h->mb.pic.p_fref[0][4],
    ...
    if( !b_force_no_skip )
    {
        if( h->mb.i_type == P_L0 && h->mb.i_partition == D_16x16 &&
            ...
            M32( h->mb.cache.mv[x264_scan8[0]] ) == M32( h->mb.cache.pskip_mv )
            && h->mb.cache.ref[x264_scan8[0]] == 0 )
        {
            h->mb.i_type = P_SKIP;
        }
 ...
 *****************************************************************************/
static  int x264_macroblock_probe_skip_internal( x264_t *h, int b_bidir, int plane_count, int chroma )
{
    ALIGNED_ARRAY_N( dctcoef, dct4x4,[8],[16] );
    ...
    for( int cat = 0; cat < 3 ; cat++ )
    {
        ...
        for( int i = 0; i < size; i++ )
            h->nr_offset[cat][i] =
              ...
              / ((uint64_t)h->nr_residual_sum[cat][i] /256 + 1);
 ...
 *****************************************************************************/
static  void x264_macroblock_encode_p8x8_internal( x264_t *h, int i8, int plane_count, int chroma )
{
    int b_decimate = h->mb.b_dct_decimate;
    int i_qp = h->mb.i_qp;
    int x = i8&1;
 ...
 *****************************************************************************/
static  void x264_macroblock_encode_p4x4_internal( x264_t *h, int i4, int plane_count )
{
    int i_qp = h->mb.i_qp;
