...
void x264_register_vid_filters( void )
{
    ...
    { extern cli_vid_filter_t cache_filter; register_vid_filter( &cache_filter );};
    { extern cli_vid_filter_t crop_filter; register_vid_filter( &crop_filter );};
    { extern cli_vid_filter_t fix_vfr_pts_filter; register_vid_filter( &fix_vfr_pts_filter );};
    { extern cli_vid_filter_t resize_filter; register_vid_filter( &resize_filter );};
    { extern cli_vid_filter_t select_every_filter; register_vid_filter( &select_every_filter );};
    { extern cli_vid_filter_t depth_filter; register_vid_filter( &depth_filter );};
#if HAVE_GPL
#endif
}
